...
- 水量は標準、計算品質は軽量、粘性は水に近い値（8）に固定。液体表示・通常速度で再生。
- ドラッグで視点を回転、ホイールまたはピンチで拡大。

## 実装と制約

`solver.js` は固定時間刻み（1/60秒）、空間格子による近傍探索、
2回の密度拘束補正、XSPHによる速度平滑化を行う簡易PBF実装です。
...
近傍の粒子の密度に応じて描画サイズを変え、水から離れた粒子は小さな水滴として
表示します。物理計算の粒子半径は変えず、見た目だけを調整しています。
液体描画は深度512px・厚み256pxまでに抑え、ぼかしの回数・範囲も制限しています。
